...
var EleMatch = Matcher;



var skin = fs.readFileSync(__dirname + '/sys/skin.html').toString();
var matcher = new EleMatch({
    'page-content': function() {
        return preq.get('https://en.wikipedia.org/api/rest_v1/page/html/Tank')
...
var server = http.createServer(function(req, res) {
    ...
    res.writeHead(200);
    matcher.matchAll(skin).forEach(function(elem) {
        if (typeof elem === 'string') {
            promise = promise.then(function() { res.write(elem); });
        } else if (elem instanceof P) {
            promise = P.join(elem, promise)
            .then(function(results) { res.write(results[0]); });
        }
    });
